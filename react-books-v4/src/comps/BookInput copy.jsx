...
function BookInput() {
   ...
   * 		let nextId = 0
   * 실제 화면전체가 변경되지 않더라도
   * 		화면전체가 변경 : refresh (새로고침)
   * 		react는 화면을 refresh 하지 않고 rendering 한다
   * 		데이터(state)가 변경되었을때만 reRendering 된다
   * 변수는 무조건 초기화 돼버린다
   *
   * 초기화 되는 것을 방지하면서 현재 화면에서 어떤 변수(public 변수)의 값을 유지하고 싶을 때가 있다
  ...
  // 타이밍에 따라 아직 UUID가 미처 추가되지 않은 상태에서 리스트에 추가되는 것을 방지하기 위하여
  ...
  const bookInsert = async () => {
    ...
    // Ref로 선언된 변수의 current 요소를 1 증가하여 다음번 list의 id로 사용한다
    nextId.current++;
    /**
     * 리스트에 추가한 후 book state를 초기화하여 입력창에 입력된 내용을 삭제하기
     * 이 기능을 사용하려면 반드시 input box에 value를 사용해야 한다
     * <input value={state} />
     *
     * <input defaultValue={state} /> 를 사용하면 기능 효과 없음
     */
