 ...
 * 입력박스에 입력된 문자열로 sample state를 변경하는 일 수행하기
 * setSample() 함수를 전달받아야 하는데
 * 그 경로가 여러겹으로 겹치는 현상이 발생한다
 *
 * React에서 state를 사용한 코딩을 할 때 매우 자주 발생하는 현상이다
 *
 * 이러한 현상을 막는 코딩을 하기 위하여
 * Redux라는 3rd 라이브러리를 많이 사용하는데
 * 우리는 Context 라는 react 기본 함수를 사용할 예정
 */
...
/**
 * PropsDrilling 컴포넌트에 선언된 sample state 값을 PropsLevel_1의 함수 컴포넌트를 사용하여 표시하고자 한다
 */
