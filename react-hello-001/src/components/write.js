 ...
 * 	컴퓨터의 fan은 무한대로 발작할 것 (이걸 막아야 한다!)
 */
 ...
 * 키보드로 이름(문자열)을 입력하면
 * 내부에서 b_name = scan.nextString() 과 유사한 코드가 실행되어
 * b_name이라는 변수에 문자열을 저장하도록 코드가 작동될 것이다
 *
 * 하지만 react 에서는 b_name = 이름 과 같은 코드는 금지된다
 * 때문에 react로 form을 만들고 value 속성에 변수명을 지정하는 순간
 * input box는 read only가 된다
 *
 * 그렇다면 input에 뭔가를 입력하고 싶은데 read only가 되버리면
 * input을 사용하는 의미가 없어진다
 * react에서는 input box에 change event를 설정하고
 * 키보드로 입력된 문자열을 가로채서 state() 함수를 사용하여 값을 변화시켜야 한다
 */
const Write = () => {
  // b_name 상태(변수)를 생성하고
  // 초기값을 서녕으로 설정하고
  ...
  const [b_name, setB_Name] = useState("서녕");
   ...
   * 2. 실제 _b_name 변수가 가리키고 있는 메모리의 주소를 b_name(임시변수) 주소로 변경한다
   * 3. 결국 원래 b_name 변수는 그대로 유지되고 새로 만들어진 _b_name 변수가 생성이 된다
   * 4. react 는 원래 b_name 변수와 새로 생성된 _b_name 변수를 비교하여 변화가 있으면 rendering을 한다
   *
   */
  const onWriterChangeHandle = (e) => {
    const b_writer_text = e.target.value;
    // react 에서는 상태(변수, 객체)에 직접 값을 할당해서는 절대 안된다
    // b_name = b_writer_text : 이렇게 쓰면 절대 안됨
  ...
  const onChangeHandle = (e) => {
     ...
     * name 변수에 담긴 값과 같은 요소를 value값을 변경하여 원래의 bbsVO와 교체하라
     *
   ...
   * 2. 현재(writer.js)에서 선언된 state 객체를 bbs라는 이름의 변수에 담아서 BBsView에게 전달하라
   */
  return (
    <form>
      <p>
        <input name="b_date" value={bbs.b_date} onChange={onChangeHandle} />
      </p>
      <p>
        <input name="b_time" value={bbs.b_time} onChange={onChangeHandle} />
      </p>
      <p>
        <input name="b_writer" value={b_name} onChange={onWriterChangeHandle} />
      </p>
      <p>
        <input
          name="b_subject"
