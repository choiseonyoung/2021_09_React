 ...
 *
 * 자원의 명확한 지정
 * 		localhost:3000/book/delete?id=12345 이런식으로 보통 요청을 수행하는데
 * 		localhost:3000/book/12345/delete : RESTFul 에서 할 일, 자원을 명확하게 설정하기
 *
 ...
 *
 * client에서 요청을 할 때
 * 같은 URL을 통하여 요청을 해도 method가 다르면 서로 다른 역할을 수행할 수 있기 때문에
 ...
 * atlas -> node -> react
 *
 * fineOne() 이 return하는 doc가 성능상 문제로 null 값이 되어 overwrite() 가 비정상 작동되므로 사용하지 말자 !!
 */
router.put("/bucket/over", async (req, res) => {
  const body = req.body;
  // DB에서 b_id 값이 body.b_id 와 같은 데이터를 SELECTE 하기
...
router.get("/update/:id/:title", (req, res) => {
  ...
  // res.status(200).end("끝") // 옛날엔 이렇게 썼음
  /**
   * Http Status Code
   ...
   * 	404 : Not found
   * 	401, 403 : 권한이 없다
   * 	405 : method가 잘못됐다 (노드에서는 이걸 보내지 않음)
   * 	400 : 보낸 데이티ㅓ가 문제가 있다
   * 5xx : Server Internal Error
   */
  // 내가 지금 너한테 데이터를 보내려고 준비를 하고 있으니 기다려라 하고 데이터를 보내는 것
  res.end("끝");
});
